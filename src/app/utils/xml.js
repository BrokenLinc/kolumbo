...
class Text extends Node {
  ...
  render(indent = 0) {
    return this.content
      .split("\n")
      .map((line) => this.indent(indent) + line)
      .join("\n");
  }
}
...
class Xml extends Node {
  ...
  render(indent = 0) {
    var attributesString = Object.keys(this.attributes)
    ...
    out.push(
      `${this.indent(indent)}<${this.tagName}${attributesString ? " " + attributesString : ""}`
    );
    ...
    new Xml(
      ...
      [
        ...
        new Xml("g", { transform: "translate(0,0)" }, [
          ...
          new Xml("polyline", {
            points: "42,37 52,37 52,77 62,77",
            id: "e2",
            class: "edge",
          }),
          new Xml("rect", {
